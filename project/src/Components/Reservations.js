   ...
   const handleDate = (e) => {
    ...
    const handleResForm = (e) => {
      ...
      const {indoorOutdoor, diners, occasion, time} = reservationDetails;
        if (ValidateContactform() && indoorOutdoor && diners && occasion && time) {
             handleConfirmswap();
...
const Formtwo = ({reservationDetails, details, onContactsChange, date, errors, onSwapping}) => {
          ...
          <main>
            ...
            {confirmswap && (
             ...
             { swap ?
              ...
              onContactsChange={handleContactsChange} date={date} errors={errors} onSwapping={handleSwap} />)
            }

            <section className="submit_btn">
                <button type="submit" onClick={handleResForm} className={`${confirmswap || successmessage ? "blurred" : " "}`}>{ swap ? "Make Reservation" : "Submit Details" }</button>
            </section>
